...
import { ArrowRight, Check, Code2, Headphones, MessageCircle, ShieldCheck, Sparkles, TrendingUp, Zap } from "lucide-react"
    ...
    <div className="relative mx-auto h-[24rem] w-full max-w-[42rem] sm:h-[32rem] lg:h-[38rem]">
      ...
      <div className="hero-float-laptop absolute bottom-[16%] left-[17%] z-10 w-[70%] transition-transform duration-700 hover:-translate-y-2">
        <div className="relative aspect-[1.45] overflow-hidden rounded-[1rem] border-[8px] border-[#173e32] bg-[#dfe9da] shadow-[0_25px_35px_rgba(18,56,42,0.3)] sm:border-[10px]">
          ...
          <div className="grid h-[calc(100%-1.75rem)] grid-cols-[0.9fr_1.1fr] gap-2 p-3 sm:h-[calc(100%-2.25rem)] sm:p-5"><div><span className="text-[7px] uppercase tracking-[0.12em] text-[#60806a] sm:text-[9px]">Digital solutions</span><strong className="mt-2 block max-w-[8rem] font-serif text-base leading-[0.95] text-[#173e32] sm:text-2xl">Build something amazing.</strong><span className="mt-3 block h-2 w-12 rounded-full bg-[#1c5b4e]" /><span className="mt-2 block h-1.5 w-20 rounded-full bg-[#c2d3bf]" /></div><div className="relative rounded bg-[#c8dac7] p-2"><div className="absolute bottom-2 left-2 right-2 h-10 rounded bg-[#739875]/70" /><div className="absolute right-2 top-2 h-12 w-16 rounded bg-[#f4f1e7]/75" /></div></div>
        </div>
      ...
      <div className="hero-float-growth absolute right-[4%] top-[10%] z-30 w-32 rotate-2 rounded-xl border border-white/70 bg-[#f8f5eb]/90 p-4 shadow-[0_18px_28px_rgba(47,75,58,0.16)] transition-transform duration-700 hover:rotate-0 hover:-translate-y-2 sm:right-[5%] sm:w-44 sm:p-5"><span className="text-[8px] uppercase tracking-[0.14em] text-[#597360] sm:text-[10px]">Growth</span><strong className="mt-1 block text-lg text-[#135744] sm:text-2xl">+312%</strong><div className="relative mt-3 h-10"><TrendingUp className="absolute bottom-0 right-0 h-9 w-9 text-[#3d966d]" /><div className="absolute bottom-1 left-0 h-px w-full rotate-[-18deg] bg-[#70bd94]" /><div className="absolute bottom-3 left-1/2 h-px w-1/2 rotate-[20deg] bg-[#70bd94]" /></div></div>

      <div className="hero-float-card absolute bottom-[28%] right-[3%] z-30 flex w-40 items-center gap-3 rounded-xl border border-white/70 bg-[#f8f5eb]/90 p-3 shadow-[0_18px_28px_rgba(47,75,58,0.16)] transition-transform duration-700 hover:-translate-y-2 sm:right-[0%] sm:w-52 sm:p-4"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#dce9d7] text-[#1c5b4e]"><Sparkles className="h-5 w-5" /></span><span className="text-[10px] font-semibold leading-4 text-[#315542] sm:text-xs">Innovative Solutions<br />for a Digital Future</span></div>

      <div className="hero-float-cup absolute bottom-[4%] right-[7%] z-20 transition-transform duration-700 hover:-translate-y-2 sm:right-[8%]"><div className="h-20 w-24 rounded-b-[2rem] rounded-t-lg bg-[#174c3c] text-center text-[10px] italic text-white shadow-[0_15px_18px_rgba(18,56,42,0.2)] sm:h-28 sm:w-36 sm:text-sm"><span className="block pt-7 font-serif">Better Tech.<br />Bigger Dreams.</span></div><div className="absolute right-[-1.3rem] top-7 h-9 w-7 rounded-r-full border-[5px] border-[#174c3c] sm:top-10 sm:h-12 sm:w-10" /><div className="mx-auto h-3 w-28 rounded-[50%] bg-[#c8a06c] sm:w-40" /></div>
    </div>
  )
}
      ...
      <div className="relative mx-auto max-w-7xl px-5 pb-8 sm:px-8 lg:px-10 lg:pb-0">
        <div className="grid items-center gap-8 lg:min-h-[38rem] lg:grid-cols-[0.94fr_1.06fr] lg:gap-3">
          <div className="relative z-10 max-w-2xl lg:pb-12">
            <span className="mb-5 inline-flex rounded-full bg-[#dfebdc] px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#35604a]">About Orvexa Systems</span>
            <h1 className="max-w-2xl font-sans text-4xl font-bold leading-[0.98] tracking-[-0.045em] text-[#063e2e] sm:text-5xl lg:text-6xl xl:text-7xl">We build digital systems that<br />move businesses forward.</h1>
            <p className="mt-6 max-w-xl text-base leading-7 text-[#587266] sm:text-lg">We&apos;re a full-service web development and digital solutions agency helping businesses build, grow, and scale with modern technology.</p>
